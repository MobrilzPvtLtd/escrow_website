...
export function ProfileView({ userType, userEmail, trustScore, onBack }: ProfileViewProps) {
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-8 mb-8">
              ...
              <Stat label="Performance" value="4.8" suffix="/ 5.0" />
              <Stat label="Trust Score" value={`${trustScore}%`} />
              {userType === 'seller' && <Stat label="Total Received" value="$12,450" />}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-xl shadow-slate-200/20">
          <SectionTitle icon={<Mail className="w-5 h-5" />} title="Contact Information" />
        ...
        {/* Security */}
        <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-xl shadow-slate-200/20">
          <SectionTitle icon={<Lock className="w-5 h-5" />} title="Security & Account" />
          <div className="space-y-4">
            <SecurityRow
              icon={<Lock className="w-5 h-5 text-slate-400 group-hover:text-slate-900 transition-colors" />}
              label="Change Password"
              sub="Update your account security"
            />
            <SecurityRow
              icon={<Bell className="w-5 h-5 text-slate-400 group-hover:text-slate-900 transition-colors" />}
              label="Notifications"
...
function ContactField({ icon, bg, label, value, isEditing, type = 'text', multiline, onChange }: ContactFieldProps) {
  const inputCls =
  ...
  return (
    <div className="flex items-start gap-4">
      <div className={`mt-1 w-8 h-8 rounded-lg ${bg} flex items-center justify-center flex-shrink-0`}>
        {icon}
      </div>
      <div className="flex-1">
        ...
        {isEditing ? (
          ...
          <p className="text-slate-900 font-bold leading-relaxed">{value}</p>
        )}
      </div>
    </div>
  );
...
function SecurityRow({ icon, label, sub }: { icon: React.ReactNode; label: string; sub: string }) {
  return (
